...
backdrop-filter: blur(10px);
        ...
        <script>
            function validate(){
                if (!(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(myForm.E_mail.value))){
 ...
 if(isset($_POST['submit']))
 {
  ...
  else
  {
   ...
   $insert=mysqli_query($conn,"INSERT INTO customer (E_mail, password, name,country,city,phone,address,card_number,gender) VALUES ('$E_mail', MD5('".$password."'),'$name','$country','$city','$phone','$address','$card_number','$gender')") or die(mysqli_error($conn));
...
?>
        ...
        <form name="myForm" action=# onsubmit="return validate()" method="post">
            <table cellspacing='5' align='center'>
                ...
                <tr><td>gender:                 <select name="gender" id="gender">
              ...
              <tr><td></td><td><input type='submit' name='submit' id="submit" value='Submit'/> <input type='button' name='back' id="back" value='back' onclick="window.location.href='http://localhost/finalproject_car/signinup/Home.html'"/> <input type="reset"></td></tr> 
                </table>
        </form>
    </body>
</html>
